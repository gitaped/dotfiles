...
compress(){
	if [ -z "$1" -o -z "$2" ]; then
		echo "Usage:"
		echo " compress path/filename zip|rar|bz2|gz|tar|tbz2|tgz|Z|7z|xz|ex|tar.bz2|tar.gz|tar.xz"
		echo
	else
		if [ -f "$2" ]; then
			case "$2" in
				tar.bz2)   tar cjSf "$1.$2" "$1" ;;
				tar.gz)    tar zcf "$1.$2" "$1"  ;;
				bz2)       bzip2 "$1.$2" "$1"     ;;
				rar)       rar a "$1.$2" "$1"     ;;
				gz)        gzip "$1.$2" "$1"      ;;
				tar)       tar cxf "$1.$2" "$1"  ;;
...
# Remove all except 1 file
rme(){
	ls | grep -v "$1" | xargs rm >/dev/null 2>&1
}
