"""""""""""""""
"vimrc        |
"Ansley Peduru|
"""""""""""""""

source $HOME/dotfiles/files/ackgrep.vim
source $HOME/dotfiles/files/autoformat.vim
source $HOME/dotfiles/files/delimitmate.vim
source $HOME/dotfiles/files/gitgutter.vim
source $HOME/dotfiles/files/indexedsearch.vim
source $HOME/dotfiles/files/mappings.vim
source $HOME/dotfiles/files/plugins.vim
source $HOME/dotfiles/files/settings.vim
source $HOME/dotfiles/files/syntastic.vim
source $HOME/dotfiles/files/whitespace.vim
source $HOME/dotfiles/files/youcompleteme.vim
source $HOME/dotfiles/files/statusline.vim
" source $HOME/dotfiles/files/notes.vim
